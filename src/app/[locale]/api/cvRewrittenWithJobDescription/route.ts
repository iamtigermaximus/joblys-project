...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < resumeData.work_experience.length; i++) {
    const rewrittenResponsibility =
      openaiApiResponses[i]?.choices?.[0]?.message?.content;
    ...
    resumeData.work_experience[i].responsibilities =
      rewrittenResponsibilitiesFormatted;
  }
